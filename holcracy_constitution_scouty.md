...
* サークル・ロールの所属関係を移動する。（サークル内のロール・サークルを外に出したり、外から別なロール・サークルを中に入れる）
* サークルの規定を定義・変更・削除する。
...
ガバナンスミーティングに出席するのは、該当サークルのSecretaryとFacilitatorを含むコアサークルメンバー全員である。
前項の通り、サークル内のロール・サークルを外に出したり、外から別なロール・サークルを中に入れる場合も、そのサークルで決議することができるので、サークルを移動する場合は、移動されるサークル・移動先のサークルのコアサークルメンバーが出席する。
これに加え、他のサークルのメンバーであっても、参加を希望すればガバナンスミーティングへの参加は認められる。
